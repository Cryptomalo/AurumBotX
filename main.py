...
try:
    ...
    st.error("Failed to load project modules. Please check the installation.")
    sys.exit(1)
...
def init_components():
    try:
        ...
        return data_loader, trading_bot
    except Exception as e:
        logger.error(f"Failed to initialize components: {e}")
        raise

try:
    if 'data_loader' not in st.session_state or 'trading_bot' not in st.session_state:
        data_loader, trading_bot = init_components()
        st.session_state.data_loader = data_loader
        st.session_state.trading_bot = trading_bot
except Exception as e:
    logger.error(f"Component initialization error: {e}")
    st.error("Failed to initialize trading components. Please try refreshing.")
...
def handle_error(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.error(f"All attempts failed for {func.__name__}: {last_error}")
        st.error(f"Operation failed after {max_retries} attempts: {str(last_error)}")
        return None
...
def start_bot(symbol, initial_balance, strategies=None):
    ...
    if symbol in running_bots:
        logger.warning(f"Bot already running for {symbol}")
    ...
    try:
        ...
        st.success(f"Bot started successfully for {symbol}")
...
def stop_bot(symbol):
    ...
    if symbol in running_bots:
        try:
            bot_info = running_bots[symbol]
            bot_info['bot'].stop()
            del running_bots[symbol]
            logger.info(f"Bot stopped successfully for {symbol}")
            st.success(f"Bot stopped for {symbol}")
        except Exception as e:
            logger.error(f"Failed to stop bot: {e}")
            raise
    else:
        logger.warning(f"No running bot found for {symbol}")
        st.warning(f"No running bot found for {symbol}")
...
try:
    ...
    with st.sidebar:
        ...
        if 'data_loader' in st.session_state:
            selected_coin = st.selectbox(
                ...
                options=list(st.session_state.data_loader.supported_coins.keys()),
                format_func=lambda x: st.session_state.data_loader.supported_coins[x]
            )
            ...
            st.markdown("---")
            total_profit = 125000  # Example (to be replaced with real data)
    ...
    if 'data_loader' in st.session_state and 'trading_bot' in st.session_state:
        # Load market data
        with st.spinner('Loading market data...'):
            try:
                df = st.session_state.data_loader.get_historical_data(selected_coin, timeframe)
                if df is not None and not df.empty:
                    ...
                    price_change = (df['Close'].iloc[-1] / df['Close'].iloc[-2] - 1) if len(df['Close']) > 1 else 0
                    ...
                    logger.info("Main chart rendered successfully")

                else:
                    logger.warning(f"No data available for {selected_coin}")
        ...
        st.error("Trading components not initialized. Please refresh the page.")
...
with prediction_tab1:
    if 'data_loader' in st.session_state and 'trading_bot' in st.session_state and df is not None and not df.empty:
        try:
            # Get predictions
            predictions = st.session_state.trading_bot.prediction_model.predict(df)
        ...
        st.warning("Please select a cryptocurrency and ensure components are initialized.")

with prediction_tab2:
    if 'trading_bot' in st.session_state and hasattr(st.session_state.trading_bot.prediction_model, 'metrics') and st.session_state.trading_bot.prediction_model.metrics:
        metrics = st.session_state.trading_bot.prediction_model.metrics
        ...
        st.warning("Prediction model metrics not available.")


with prediction_tab3:
    st.subheader("Importanza delle Features")
    if 'trading_bot' in st.session_state:
        feature_importance = st.session_state.trading_bot.prediction_model.get_feature_importance()
        ...
        st.warning("Trading bot not initialized.")
...
with tab1:
    ...
    with st.spinner('Loading market data...'):
        if 'data_loader' in st.session_state and selected_coin:
            try:
                logger.info(f"Loading data for {selected_coin}")
                df = st.session_state.data_loader.get_historical_data(selected_coin, timeframe)
    ...
    if st.button("🔄 Run Strategy Simulation", use_container_width=True):
        try:
            with st.spinner("Running trading simulation..."):
                ...
                if 'trading_bot' in st.session_state:
                    st.session_state.trading_bot.train(df)
                    predictions = st.session_state.trading_bot.predict(df)
...
with tab3:
    ...
    try:
        ...
        def setup_websocket_connection():
            ...
            for attempt in range(max_retries):
                try:
                    # Setup WebSocket connection - Placeholder, needs actual implementation
